...
class FXDefaultLinks {
	private $knownFormatting = array(); /* Page ID/Title or 'Page ID#fragment' => link text (retrieval cache) */
	private $supressedOptions = array(); /* ParserOptions objects for which default links are disabled */
	...
	public static function onParserFirstCallInit(Parser &$parser) {
		...
		if ($instance == null) {
			global $wgHooks;
			$instance = new FXDefaultLinks;
			$wgHooks['InternalParseBeforeSanitize'][] = $instance;
			$wgHooks['ParserBeforeInternalParse'][] = $instance;
		}
		$parser->setFunctionHook( 'defaultlink', array($instance, 'linkParserFunction'), SFH_NO_HASH | SFH_OBJECT_ARGS);
		$parser->setHook('nodefaultlinks', array($instance, 'noLinksTag'));
		return true;
	}
	...
	private static function deleteProps($page_id) {
		wfGetDB(DB_MASTER)->delete('page_props', array('pp_page' => $page_id, 'pp_propname' => array('defaultlink', 'defaultlinksec')), __METHOD__);
	}
	...
	public static function onLinksUpdateConstructed(LinksUpdate &$lu) {
		if (isset($lu->mProperties) && isset($lu->mProperties['defaultlinksec'])) {
			$s = '';
			foreach ($lu->mProperties['defaultlinksec'] as $k => $v) $s .= ($s == '' ? '' : "\n").$k."\n".$v;
	...
	public function onInternalParseBeforeSanitize(&$parser, &$text, &$stripState) {
		...
		if (MagicWord::get( 'nodefaultlink' )->matchAndRemove( $text )) {
		...
		$lookup = array(); $lock = array(); $lookupReplace = array();
		...
		foreach ($linkMatches[1] as $key => $target) {
			...
			if (strpos($target, '%') !== false) {
				$target = str_replace(array('<', '>'), array('&lt;', '&gt;'), urldecode($target));
			}
			$lock[$whole] = true;
			...
			if (($fragment = strtolower($title->getFragment())) != '') {
				...
				if ($page_id == $this_page_id && $this_page_slinks && isset($this_page_slinks[$fragment])) {
					$this->knownFormatting[$hkey] = $this_page_slinks[$fragment];
				}
				if (isset($this->knownFormatting[$hkey])) {
					$find[] = $whole;
					$replace[] = $this->knownFormatting[$hkey];
				} elseif (isset($this->knownFormatting[$page_id]) || ($page_id == $this_page_id && !$parser->getOptions()->getIsSectionPreview())) {
			...
			foreach ($fmtRes as $row) {
				if ($row->pp_propname == 'defaultlinksec')	{
					...
					for ($i = 0, $c = count($links)-1; $i < $c; $i+=2) {
						$this->knownFormatting[$row->pp_page . '#' . strtolower(trim($links[$i]))] = $links[$i+1];
					}
				}	else {
	...
	public function linkParserFunction(&$parser, $frame, $args) {
		...
		$silent = isset($args[2]) ? MagicWord::get( 'silent' )->match($frame->expand($args[2])) : false;
		$hasLinks = self::nsHasFormattedLinks($title);
		if ($silent && !$hasLinks) return '';
		
		$thisTitle = $title->getPrefixedText();
		$forPage = isset($args[1]) ? trim($frame->expand($args[1])) : '';
		$fragment = '';
		if (strpos($forPage, '%') !== false) $forPage = str_replace(array('<', '>'), array('&lt;', '&gt;'), urldecode($forPage));

		if ($forPage != '') { // Check whether this tag is meant for this page
			$forTitle = Title::newFromText($forPage);
			if (!is_object($forTitle)) return $silent ? '' : '<span class="error">'.wfMessage('defaultlink-target-page-invalid')->text().'</span>';
			if ($forTitle->getPrefixedText() != '' && $forTitle->getPrefixedText() != $thisTitle) return '';
			$forPage = $forTitle->getPrefixedText();
			$fragment = str_replace("\n", '', $forTitle->getFragment());
		}
		$link = isset($args[0]) ? trim($frame->expand($args[0])) : '';
		if (!is_string($link) || !preg_match('/\[\[.*\]\]/s', $link)) return $silent ? '' : '<span class="error">'.wfMessage('defaultlink-invalid-link')->text().'</span>';
		...
		foreach($linkMatches[1] as $key => $target) {
			...
			if (!is_object($tarTitle)) continue;
			$targetTitle = $tarTitle->getPrefixedText();
			if ($tarTitle->getNamespace() == NS_FILE && $target[0] != ':') {
				if (preg_match('/\|\s*link=\s*([^|\\]]+)/', $linkMatches[2][$key], $fileLinkMatch)) {
					$fileLinkTitle = Title::newFromText($fileLinkMatch[1]);
					$targetTitle = is_object($fileLinkTitle) ? $fileLinkTitle->getPrefixedText() : $targetTitle;
				}
			}
			if ($thisTitle == $targetTitle) {
				if (!$hasLinks) return '<span class="error">'.wfMessage('defaultlink-disallowed-namespace')->text().'</span>';
				if ($fragment == '') {
					$oldLink = $parser->getOutput()->getProperty('defaultlink');
					$parser->getOutput()->setProperty('defaultlink', $link);
					if ($oldLink !== false && trim($oldLink) != trim($link)) return '<span class="error">'.wfMessage('duplicate-defaultlink', $of, $link)->text().'</span>';
				} else {
					$oldLinks = $parser->getOutput()->getProperty('defaultlinksec');
					if ($oldLinks == false) $oldLinks = array();
	...
	public function onParserBeforeInternalParse( &$parser, &$text, &$stripState) {
		...
		if (MagicWord::get( 'nodefaultlink' )->match( $pure )) {
			$this->supressedOptions[] = $parser->getOptions();
		}
		return true;
	}
